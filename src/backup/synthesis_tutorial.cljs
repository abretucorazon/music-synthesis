...
(defn control-ui [onClick]
  (dom/div #js {:className "controls"}
          (dom/button #js {:onClick onClick} "Test")))
...
(defn render-one [k handle! state]
  (let [[text code] (steps k)]
    (dom/div nil
     ...
     (control-ui #(handle! (action/->Test k))))))
  ...
  (render [this]
    (let [{:keys [handle! state]} (om/props this)]
      (dom/div nil
       ...
       (dom/div nil
        (dom/p nil "Now that you know how to design synthesisers, try "
               (dom/a #js {:href "/klangmeister/composition"} "composing melodies") "."))))))

(def synth-ui (om/factory  Synth-ui))
